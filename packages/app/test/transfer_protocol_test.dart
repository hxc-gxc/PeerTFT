...
class _FakeWebWritableSink implements WebWritableSink {
  ...
  @override
  Future<void> write(Uint8List chunk) async => writes.add(chunk);

  @override
  Future<void> close() async => closeCalls++;

  @override
  Future<void> abort() async => abortCalls++;
}
...
void main() {
  group('FileSender / FileReceiver protocol', () {
    ...
    test(
      'resume: receiver seeded with initialBytes gets full file back',
      () async {
        final full = Uint8List.fromList(
          List.generate(40 * 1024, (i) => i % 256),
        );
    ...
    test(
      'receiver.onMeta fires once on a fresh receive with name and size',
      () async {
        String? gotName;
        int? gotSize;
        final (:sender, :receiver) = _makePipe(
